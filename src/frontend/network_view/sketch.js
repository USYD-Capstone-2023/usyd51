...
class Device {
  ...
  addLink(device) {
    ...
    for (let i = -1; i <= lineSegments + 1; i++) {
      newLine.push([
        lerp(this.x, device.x, i / lineSegments),
        sigmoidInterp(this.y, device.y, i / lineSegments),
      ]);
    }
  ...
  drawDataBox() {
    ...
    if (!keyIsDown(SHIFT)) {
      ...
      document.getElementById("deviceMACAddress").innerText =
        this.data.mac || "N/A";
  ...
  hoverCheck() {
    ...
    if (
      Math.pow(rmouseX() - this.x, 2) + Math.pow(rmouseY() - this.y, 2) <
...
function setup() {
  createCanvas(
    windowWidth,
    windowHeight - document.getElementById("header").offsetHeight
  );
  frameRate(60);
}
...
function mouseDragged() {
  if (keyIsDown(SHIFT)) {
    if (deviceFound == undefined) {
      for (device of allDevices) {
        if (
          Math.pow(rmouseX() - device.x, 2) +
            Math.pow(rmouseY() - device.y, 2) <
          Math.pow(device.radius, 2)
        ) {
          deviceFound = device;
    ...
    for (let i = 0; i < levels_array[layer].length; i++) {
      ...
      allDevices.push(
        new Device(
          ...
          y_pos,
          10,
          data[levels_array[layer][i]],
          data[levels_array[layer][i]].name
        )
      );
    }
  }
